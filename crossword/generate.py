...
class CrosswordCreator():
    ...
    def order_domain_values(self, var, assignment):
        ...
        for value in self.domains[var]:
            ...
            scores[value] = constraining_count

        scores = sorted(scores, key=lambda x: scores[x])
    ...
    def select_unassigned_variable(self, assignment):
        ...
        mrv_candidates = [var for var in unassigned_vars if len(
            self.domains[var]) == min_domain_size]

        if len(mrv_candidates) > 1:
            mrv_candidates.sort(key=lambda var: len(self.crossword.neighbors(var)), reverse=True)
